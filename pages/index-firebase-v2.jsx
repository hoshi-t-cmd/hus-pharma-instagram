...
// Drive upload - returns thumbnail URL too
...
// Get Drive file thumbnail via API
...
function SubmitForm({user, photos, addPhoto, onLogout}) {
  ...
  const [preview,setPreview] = useState(null); // local preview only
  ...
  async function submit() {
    if (!token) { setUploadError("Googleと連携してからもう一度試してください"); return; }
    setUploading(true); setUploadError("");
    let dr = null;
    try {
      dr = await uploadToDrive(file, fileName, FOLDER_IDS[catId]||DRIVE_ROOT_FOLDER_ID, token);
    } catch(e) {
      setUploadError(`Drive保存失敗: ${e.message}`);
      setUploading(false); return;
    }
    // Firestoreには軽いデータのみ保存（画像データは含めない）
    await addPhoto({
      ...
      fileName, drivePath,
      driveFileId:  dr.id,
      driveLink:    dr.webViewLink,
      driveThumbnail: getDriveThumbnail(dr.id),
      submittedAt: nowJST(),
  ...
  function reset() {
    ...
    setDt(()=>{const n=new Date();return new Date(n.getTime()+9*3600000).toISOString().slice(0,16);});
  }
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F5F5F3",fontFamily:"'Noto Sans JP','Hiragino Sans',sans-serif"}}>
      ...
      <header style={{background:"#fff",borderBottom:"1px solid #EBEBEB",padding:"0 12px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:20}}>💊</span>
          <div><div style={{fontSize:12,fontWeight:900}}>薬学部 写真共有</div><div style={{fontSize:10,color:"#888"}}>📸 {user.name}</div></div>
        </div>
        <div style={{display:"flex",gap:5}}>
          {!token
            ? <button onClick={signIn} style={{fontSize:11,padding:"5px 8px",borderRadius:6,border:"1.5px solid #3D7A5A",background:"#EEF7F2",color:"#3D7A5A",cursor:"pointer",fontWeight:700}}>🔗 Google連携</button>
            : <span style={{fontSize:11,padding:"5px 8px",borderRadius:6,background:"#EEF7F2",color:"#3D7A5A",fontWeight:700}}>✅ 連携中</span>
          }
          <button onClick={()=>setView("mypage")} style={{fontSize:11,padding:"5px 8px",borderRadius:6,border:"1.5px solid #EEE",background:view==="mypage"?"#1A1A1A":"#fff",color:view==="mypage"?"#fff":"#555",cursor:"pointer"}}>👤 マイページ</button>
          <button onClick={()=>{setView("form");reset();}} style={{fontSize:11,padding:"5px 8px",borderRadius:6,border:"1.5px solid #EEE",background:view==="form"?"#1A1A1A":"#fff",color:view==="form"?"#fff":"#555",cursor:"pointer"}}>📸 投稿</button>
          <button onClick={onLogout} style={{fontSize:11,padding:"5px 8px",borderRadius:6,border:"1.5px solid #EEE",background:"#fff",color:"#888",cursor:"pointer"}}>ログアウト</button>
        </div>
      </header>

      {!token && view==="form" && (
        <div style={{background:"#FFF0F0",borderLeft:"4px solid #C44",padding:"10px 16px",fontSize:13,color:"#C44",display:"flex",alignItems:"center",gap:8}}>
          ⚠️ <strong>Googleと連携してください。</strong>連携しないと写真を送信できません。
          <button onClick={signIn} style={{fontSize:12,padding:"4px 12px",borderRadius:6,border:"1.5px solid #C44",background:"#fff",color:"#C44",cursor:"pointer",fontFamily:"inherit"}}>連携する</button>
        </div>
      )}
      ...
      {view==="mypage" && (
        <div className="fi" style={{maxWidth:480,margin:"0 auto",padding:"24px 16px 80px"}}>
          ...
          <div style={{background:"#fff",borderRadius:14,border:"1.5px solid #EEE",padding:"14px 16px",marginBottom:14}}>
            ...
            {view==="changepw"&&<div style={{marginTop:10}}>
            ...
            : <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {myPhotos.map(p=>(
                  <div key={p.id} style={{background:"#fff",borderRadius:12,border:"1.5px solid #EEE",display:"flex",gap:12,padding:"12px 14px"}}>
                    {p.driveThumbnail
                      ? <img src={p.driveThumbnail} alt="" style={{width:52,height:70,objectFit:"cover",borderRadius:7,flexShrink:0}} onError={e=>e.target.style.display="none"}/>
                      : <div style={{width:52,height:70,borderRadius:7,background:"#F0F0F0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>📷</div>
                    }
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                        <span style={{fontSize:12,fontWeight:700}}>{p.catEmoji} {p.catLabel}</span>
                        <span style={{fontSize:11,padding:"2px 8px",borderRadius:10,background:SC[p.status]+"20",color:SC[p.status],fontWeight:700}}>{SL[p.status]}</span>
                      </div>
                      {p.sub&&<div style={{fontSize:11,color:"#888",marginBottom:2}}>{p.sub}</div>}
                      <div style={{fontSize:11,color:"#AAA"}}>{p.submittedAt}</div>
                      {p.driveLink&&<a href={p.driveLink} target="_blank" rel="noreferrer" style={{fontSize:11,color:"#3D7A5A",textDecoration:"none",marginTop:3,display:"inline-block"}}>📁 Driveで見る →</a>}
        ...
        <div className="fi" style={{maxWidth:420,margin:"0 auto",padding:"0 0 80px"}}>
          ...
          {step===1&&(
            <div style={{padding:"24px 20px"}}>
              <div style={{background:"#EEF7F2",borderRadius:10,padding:"10px 14px",marginBottom:18,display:"flex",alignItems:"center",gap:8,fontSize:13}}>
                <span style={{fontSize:18}}>👤</span><span><strong>{user.name}</strong>（{user.id}）として送信されます</span>
              </div>
              <h2 style={{fontSize:20,fontWeight:800,marginBottom:6}}>📸 写真を選ぶ</h2>
              <p style={{fontSize:13,color:"#666",marginBottom:18,lineHeight:1.6}}>カメラ撮影またはフォトライブラリから選択できます</p>

              {/* ── 2ボタン：カメラ撮影 / フォトライブラリ ── */}
              <input ref={fileRef} type="file" accept="image/*" onChange={onFile} style={{display:"none"}}/>
              <input id="camera-input" type="file" accept="image/*" capture="environment" onChange={onFile} style={{display:"none"}}/>

              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
                <button onClick={()=>document.getElementById("camera-input").click()} style={{aspectRatio:"1",borderRadius:16,border:"2px solid #3D7A5A",background:"#EEF7F2",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,color:"#3D7A5A",fontFamily:"inherit"}}>
                  <span style={{fontSize:36}}>📷</span>
                  <span style={{fontSize:13,fontWeight:700}}>カメラで撮影</span>
                </button>
                <button onClick={()=>fileRef.current?.click()} style={{aspectRatio:"1",borderRadius:16,border:"2px solid #1A1A1A",background:"#F7F7F7",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,color:"#1A1A1A",fontFamily:"inherit"}}>
                  <span style={{fontSize:36}}>🖼️</span>
                  <span style={{fontSize:13,fontWeight:700}}>フォトライブラリ</span>
                </button>
              </div>
              <p style={{fontSize:11,color:"#AAA",textAlign:"center"}}>縦構図の写真を選んでください</p>
            </div>
          )}
            ...
            <div style={{padding:"20px 20px"}}>
              ...
              <Field label="カテゴリ" req>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {CATEGORIES.map(c=><button key={c.id} onClick={()=>{setCatId(c.id);setSub("");}} style={{padding:"10px 8px",borderRadius:10,border:`2px solid ${catId===c.id?"#1A1A1A":"#E0E0E0"}`,background:catId===c.id?"#1A1A1A":"#fff",color:catId===c.id?"#fff":"#333",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",textAlign:"left",display:"flex",alignItems:"center",gap:5}}><span>{c.emoji}</span>{c.label}</button>)}
                </div>
              </Field>
          ...
          {step===3&&(
            <div style={{padding:"20px 20px"}}>
              ...
              {!token&&<div style={{background:"#FFF0F0",borderRadius:10,padding:10,marginBottom:10,fontSize:12,color:"#C44"}}>⚠️ Googleと連携していません。戻って連携してください。</div>}
              ...
              <button onClick={submit} disabled={!allChecked||uploading||!token} style={{...S.btn,background:allChecked&&!uploading&&token?"#3D7A5A":"#CCC",cursor:allChecked&&!uploading&&token?"pointer":"not-allowed",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          ...
          {step===4&&(
            <div style={{padding:"60px 20px",textAlign:"center"}}>
              ...
              <p style={{fontSize:14,color:"#666",lineHeight:1.8,marginBottom:20}}>教員が確認後、投稿判断を行います。<br/>ご協力ありがとうございます！</p>
              {uploadResult&&<div style={{background:"#EEF7F2",borderRadius:12,padding:16,marginBottom:20,textAlign:"left"}}>
                <div style={{fontSize:13,fontWeight:700,color:"#3D7A5A",marginBottom:8}}>✅ Google Drive に保存されました</div>
                <div style={{fontSize:11,color:"#555",marginBottom:4}}>📂 {drivePath}</div>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={reset} style={{...S.btn,background:"#1A1A1A",flex:1}}>続けて提供する</button>
            ...
            {filtered.map(p=>(
              <div key={p.id} style={{background:"#fff",borderRadius:14,border:`1.5px solid ${selected?.id===p.id?"#1A1A1A":"#EEE"}`,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,.05)"}}>
                <div style={{display:"flex",gap:12,padding:"14px 14px 0"}}>
                  {p.driveThumbnail
                    ? <img src={p.driveThumbnail} alt="" style={{width:72,height:96,objectFit:"cover",borderRadius:8,flexShrink:0}} onError={e=>e.target.style.display="none"}/>
                    : <div style={{width:72,height:96,borderRadius:8,background:"#F0F0F0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,flexShrink:0}}>📷</div>
                  }
                  <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{padding:"0 14px 14px"}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button onClick={()=>deletePhoto(p.id)} style={{...S.btn,background:"#FFF",color:"#CCC",border:"1.5px solid #EEE",padding:"10px 12px",width:"auto",fontSize:16}}>🗑</button>
                  </div>}
                </div>
              </div>
            ))}
